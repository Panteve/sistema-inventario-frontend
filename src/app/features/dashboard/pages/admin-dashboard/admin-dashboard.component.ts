...
import { DashboardSummary } from '../../../../shared/interfaces/dashboard.interfacce';
...
export class AdminDashboardComponent implements OnInit {
  ...
  queryParams = signal<AdminDashboardFilters>(this.#buildDefaultParams());
  ...
  changeStartDate(event: any) {
    const startDate = this.#coerceIsoDate(event.detail);
    if (!startDate) return;
    this.queryParams.update((filters) =>
      this.#normalizeDateRange({
        ...filters,
        startDate,
      }),
  ...
  changeEndDate(event: any) {
    const endDate = this.#coerceIsoDate(event.detail);
    if (!endDate) return;
    this.queryParams.update((filters) =>
      this.#normalizeDateRange({
        ...filters,
  ...
  isTodayRange(): boolean {
    return (
      this.queryParams().startDate === this.todayIso && this.queryParams().endDate === this.todayIso
    );
  }
  ...
  isCurrentWeekRange(): boolean {
    ...
    return (
      this.queryParams().startDate === weekStart && this.queryParams().endDate === this.todayIso
    );
  }
  ...
  isCurrentMonthRange(): boolean {
    ...
    return (
      this.queryParams().startDate === monthStart && this.queryParams().endDate === this.todayIso
    );
  }
  ...
  applyFilters() {
    ...
    this.#dashboardService
      .getDashboardSummary(this.queryParams())
  ...
  #buildDefaultParams(): AdminDashboardFilters {
  ...
  #normalizeDateRange(filters: AdminDashboardFilters): AdminDashboardFilters {
  ...
  #loadSavedFilters(): AdminDashboardFilters | null {
    try {
      const raw = localStorage.getItem(this.#filtersStorageKey);
      if (!raw) return null;
      const parsed = JSON.parse(raw) as Partial<AdminDashboardFilters>;
      const startDate =
